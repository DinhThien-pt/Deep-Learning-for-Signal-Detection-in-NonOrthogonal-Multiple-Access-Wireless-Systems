...
% Load neural networks with different learning rates for 16 pilots
load('NNCP12_16pilots_lr0001.mat','net'); net_lr0001_16 = net; % Learning rate 0.001
load('NNCP12_16pilots_lr001.mat','net'); net_lr001_16 = net;   % Learning rate 0.01
load('NNCP12_16pilots_lr005.mat','net'); net_lr005_16 = net;   % Learning rate 0.05
load('NNCP12_16pilots_lr01.mat','net'); net_lr01_16 = net;     % Learning rate 0.1

% Load neural networks with different learning rates for 64 pilots (optional)
load('NNCP12_64pilots_lr0001.mat','net'); net_lr0001_64 = net;
load('NNCP12_64pilots_lr001.mat','net'); net_lr001_64 = net;
load('NNCP12_64pilots_lr005.mat','net'); net_lr005_64 = net;
load('NNCP12_64pilots_lr01.mat','net'); net_lr01_64 = net;
...
% Noise computation
EsN0_dB = 0:2:28; % Start from 0 dB to match Fig. 7 range
EsN0 = 10.^(EsN0_dB./10);
...
% For 16 pilots
numErr_DL_16_lr0001 = zeros(numUE,numel(EsN0_dB),ITER);
numErr_DL_16_lr001 = zeros(numUE,numel(EsN0_dB),ITER);
numErr_DL_16_lr005 = zeros(numUE,numel(EsN0_dB),ITER);
numErr_DL_16_lr01 = zeros(numUE,numel(EsN0_dB),ITER);
% For 64 pilots (optional)
numErr_DL_64_lr0001 = zeros(numUE,numel(EsN0_dB),ITER);
numErr_DL_64_lr001 = zeros(numUE,numel(EsN0_dB),ITER);
numErr_DL_64_lr005 = zeros(numUE,numel(EsN0_dB),ITER);
numErr_DL_64_lr01 = zeros(numUE,numel(EsN0_dB),ITER);
...
for it = 1:ITER
    for snr = 1:numel(nVar)
        ...
        % DL detection for 16 pilots with different learning rates
        decOrder_sc = squeeze(decOrder_all(idx_sc,:,:));
        [numErr_DL_16_lr0001(:,snr,it), ~] = symbolDecodeDL(labelClass,receivePacket_16,tLabel,net_lr0001_16,decOrder_sc,symClass,constQPSK);
        [numErr_DL_16_lr001(:,snr,it), ~] = symbolDecodeDL(labelClass,receivePacket_16,tLabel,net_lr001_16,decOrder_sc,symClass,constQPSK);
        [numErr_DL_16_lr005(:,snr,it), ~] = symbolDecodeDL(labelClass,receivePacket_16,tLabel,net_lr005_16,decOrder_sc,symClass,constQPSK);
        [numErr_DL_16_lr01(:,snr,it), ~] = symbolDecodeDL(labelClass,receivePacket_16,tLabel,net_lr01_16,decOrder_sc,symClass,constQPSK);
        
        % DL detection for 64 pilots with different learning rates (optional)
        [numErr_DL_64_lr0001(:,snr,it), ~] = symbolDecodeDL(labelClass,receivePacket_64,tLabel,net_lr0001_64,decOrder_sc,symClass,constQPSK);
        [numErr_DL_64_lr001(:,snr,it), ~] = symbolDecodeDL(labelClass,receivePacket_64,tLabel,net_lr001_64,decOrder_sc,symClass,constQPSK);
        [numErr_DL_64_lr005(:,snr,it), ~] = symbolDecodeDL(labelClass,receivePacket_64,tLabel,net_lr005_64,decOrder_sc,symClass,constQPSK);
        [numErr_DL_64_lr01(:,snr,it), ~] = symbolDecodeDL(labelClass,receivePacket_64,tLabel,net_lr01_64,decOrder_sc,symClass,constQPSK);
    end
end
toc;

% Average over iterations
numErr_DL_16_lr0001 = mean(numErr_DL_16_lr0001,3);
numErr_DL_16_lr001 = mean(numErr_DL_16_lr001,3);
numErr_DL_16_lr005 = mean(numErr_DL_16_lr005,3);
numErr_DL_16_lr01 = mean(numErr_DL_16_lr01,3);
numErr_DL_64_lr0001 = mean(numErr_DL_64_lr0001,3);
numErr_DL_64_lr001 = mean(numErr_DL_64_lr001,3);
numErr_DL_64_lr005 = mean(numErr_DL_64_lr005,3);
numErr_DL_64_lr01 = mean(numErr_DL_64_lr01,3);

% Plot SER curves for DNN trained with different learning rates (Fig. 7)
figure();
semilogy(EsN0_dB, numErr_DL_16_lr0001(1,:), 'b-d', 'LineWidth', 1.5, 'MarkerSize', 6, 'DisplayName', 'User 1, Learning rate = 0.001'); hold on;
semilogy(EsN0_dB, numErr_DL_16_lr001(1,:), 'g-d', 'LineWidth', 1.5, 'MarkerSize', 6, 'DisplayName', 'User 1, Learning rate = 0.01'); hold on;
semilogy(EsN0_dB, numErr_DL_16_lr005(1,:), 'k-d', 'LineWidth', 1.5, 'MarkerSize', 6, 'DisplayName', 'User 1, Learning rate = 0.05'); hold on;
semilogy(EsN0_dB, numErr_DL_16_lr01(1,:), 'r-d', 'LineWidth', 1.5, 'MarkerSize', 6, 'DisplayName', 'User 1, Learning rate = 0.1'); hold on;
semilogy(EsN0_dB, numErr_DL_16_lr0001(2,:), 'b-*', 'LineWidth', 1.5, 'MarkerSize', 6, 'DisplayName', 'User 2, Learning rate = 0.001'); hold on;
semilogy(EsN0_dB, numErr_DL_16_lr001(2,:), 'g-*', 'LineWidth', 1.5, 'MarkerSize', 6, 'DisplayName', 'User 2, Learning rate = 0.01'); hold on;
semilogy(EsN0_dB, numErr_DL_16_lr005(2,:), 'k-*', 'LineWidth', 1.5, 'MarkerSize', 6, 'DisplayName', 'User 2, Learning rate = 0.05'); hold on;
semilogy(EsN0_dB, numErr_DL_16_lr01(2,:), 'r-*', 'LineWidth', 1.5, 'MarkerSize', 6, 'DisplayName', 'User 2, Learning rate = 0.1'); hold off;
title('SER curves of DNN trained with different learning rates');
xlabel('SNR (dB)');
...
% Save the plot
saveas(gcf, 'Figure7_SER_vs_SNR_DNN_LearningRates.png');
