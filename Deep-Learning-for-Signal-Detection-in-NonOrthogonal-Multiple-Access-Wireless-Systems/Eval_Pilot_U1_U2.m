...
for it = 1:ITER
    for snr = 1:numel(nVar)
        ...
        [receivePacket_16, randomPhase_16] = dataTransmissionReception(transmitPacket_16, powerFactor_all, lengthCP, h_all, nVar(snr));
        receivePilot_16 = receivePacket_16(1:2, :, :, :); 
        receiveData_16 = receivePacket_16(end, :, :, :);
        
        % Received packets with random phase for 64 pilots
        [receivePacket_64, randomPhase_64] = dataTransmissionReception(transmitPacket_64, powerFactor_all, lengthCP, h_all, nVar(snr));
        ...
        [numErr_ML_16(:, snr, it), ~] = detectML(H_sc, randomPhase_16, constQPSK, pF_sc, rData_16, idx_1, idx_2, tData, symClass);
        
        % ML detection (with perfect CSI) for 64 pilots
        rData_64 = squeeze(receiveData_64(1, idx_sc, :)); % numPacket x 1
        [numErr_ML_64(:, snr, it), ~] = detectML(H_sc, randomPhase_64, constQPSK, pF_sc, rData_64, idx_1, idx_2, tData, symClass);
...
% Plot SER for User 1 with all methods for 16 and 64 pilots
figure();
...
semilogy(EsN0_dB, numErr_ML_16(1, :), 'g-o', 'LineWidth', 1.5, 'DisplayName', '16 Pilots, ML'); hold on;
semilogy(EsN0_dB, numErr_DL_64(1, :), 'r--o', 'LineWidth', 1.5, 'DisplayName', '64 Pilots, DL'); hold on;
...
semilogy(EsN0_dB, numErr_ML_64(1, :), 'g--o', 'LineWidth', 1.5, 'DisplayName', '64 Pilots, ML'); hold off;
...
saveas(gcf, 'SER_User1_Pilot16_vs_Pilot64.png');

% Plot SER for User 2 with all methods
figure();
semilogy(EsN0_dB, numErr_DL_16(2, :), 'r-o', 'LineWidth', 1.5, 'DisplayName', '16 Pilots, DL'); hold on;
semilogy(EsN0_dB, numErr_SIC_LS_16(2, :), 'b-o', 'LineWidth', 1.5, 'DisplayName', '16 Pilots, SIC with LS'); hold on;
semilogy(EsN0_dB, numErr_SIC_MMSE_16(2, :), 'k-o', 'LineWidth', 1.5, 'DisplayName', '16 Pilots, SIC with MMSE'); hold on;
semilogy(EsN0_dB, numErr_ML_16(2, :), 'g-o', 'LineWidth', 1.5, 'DisplayName', '16 Pilots, ML'); hold on;
semilogy(EsN0_dB, numErr_DL_64(2, :), 'r--o', 'LineWidth', 1.5, 'DisplayName', '64 Pilots, DL'); hold on;
semilogy(EsN0_dB, numErr_SIC_LS_64(2, :), 'b--o', 'LineWidth', 1.5, 'DisplayName', '64 Pilots, SIC with LS'); hold on;
semilogy(EsN0_dB, numErr_SIC_MMSE_64(2, :), 'k--o', 'LineWidth', 1.5, 'DisplayName', '64 Pilots, SIC with MMSE'); hold on;
semilogy(EsN0_dB, numErr_ML_64(2, :), 'g--o', 'LineWidth', 1.5, 'DisplayName', '64 Pilots, ML'); hold off;
title('SER curves for User 2 with 16 and 64 Pilot Symbols');
xlabel('SNR (dB)');
ylabel('Symbol Error Rate (SER)');
legend('show', 'Location', 'southwest');
grid on;
xlim([0 28]); % Limit x-axis to exactly 26 dB to match data
xticks(0:2:28); % Thêm nhãn rõ ràng đến 28
saveas(gcf, 'SER_User2_Pilot16_vs_Pilot64.png');
