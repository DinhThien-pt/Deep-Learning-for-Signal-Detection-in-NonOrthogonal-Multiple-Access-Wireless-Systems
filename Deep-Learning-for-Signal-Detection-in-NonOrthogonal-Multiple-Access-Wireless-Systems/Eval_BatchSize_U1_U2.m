...
if ~isequal(h, h) || ~isequal(idx_sc, idx_sc)
    error('Biến h hoặc idx_sc không nhất quán giữa 64pilots và 16pilots.');
end

% Load neural networks with different batch sizes for 64 pilots
...
% For 64 pilots with different batch sizes
...
for it = 1:ITER
    for snr = 1:numel(nVar)
        ...
        % DL detection for 64 pilots with different batch sizes
...
% Plot SER curves for User 1 and User 2 with different batch sizes (Fig. 8)
figure();
semilogy(EsN0_dB, numErr_DL_64_bs2000(1,:), 'g-d', 'LineWidth', 1.5, 'MarkerSize', 6, 'DisplayName', 'User 1, Batch size = 2000'); hold on;
semilogy(EsN0_dB, numErr_DL_64_bs5000(1,:), 'b-d', 'LineWidth', 1.5, 'MarkerSize', 6, 'DisplayName', 'User 1, Batch size = 5000'); hold on;
semilogy(EsN0_dB, numErr_DL_64_bs10000(1,:), 'r-d', 'LineWidth', 1.5, 'MarkerSize', 6, 'DisplayName', 'User 1, Batch size = 10000'); hold on;
semilogy(EsN0_dB, numErr_DL_64_bs20000(1,:), 'k-d', 'LineWidth', 1.5, 'MarkerSize', 6, 'DisplayName', 'User 1, Batch size = 20000'); hold on;
semilogy(EsN0_dB, numErr_DL_64_bs2000(2,:), 'g-o', 'LineWidth', 1.5, 'MarkerSize', 6, 'DisplayName', 'User 2, Batch size = 2000'); hold on;
semilogy(EsN0_dB, numErr_DL_64_bs5000(2,:), 'b-o', 'LineWidth', 1.5, 'MarkerSize', 6, 'DisplayName', 'User 2, Batch size = 5000'); hold on;
semilogy(EsN0_dB, numErr_DL_64_bs10000(2,:), 'r-o', 'LineWidth', 1.5, 'MarkerSize', 6, 'DisplayName', 'User 2, Batch size = 10000'); hold on;
semilogy(EsN0_dB, numErr_DL_64_bs20000(2,:), 'k-o', 'LineWidth', 1.5, 'MarkerSize', 6, 'DisplayName', 'User 2, Batch size = 20000'); hold off;
title('SER curves of DNN trained with different batch sizes for User 1 and User 2');
xlabel('SNR (dB)');
...
% Save the plot
saveas(gcf, 'SER_vs_SNR_DNN_BatchSizes.png');
